...
***There are `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each row represents the data for one Denny's restaurant. The variables are `r names(dennys)`.***

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?
    
***There are `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each row represents the data for one La Quintas Inns. The variables are `r names(laquinta)`.***
...
***La Quinta hotels are mostly located in the Americas but also have locations in other parts of the world. They are located in the U.S., Canada, Mexico, Ecuador, Columbia, China, Turkey, Chile, United Arab Emirates, and New Zealand. Denny's are located only in the U.S.***
...
***We could use the State variable to determine whether a location is in the U.S. by comparing the state listed to a database of states in the U.S. We could also see if the state abbreviations are similar to those of country abbreviations.***
...
***La Quinta is located in Canada, Mexico, Honduras, Chile, Columbia, Turkey, and New Zealand. (https://www.google.com/search?client=firefox-b-1-d&q=where+is+la+quinta+located+outside+of+the+us.)***
...
***The state with the fewest Denny's restaurants is Delaware. the state with the fewest La Quinta Inns is Maine. I don't know how to find this suprising because they are just states. Maybe because Delaware is a small state and Maine is farther out of the U.S. I suppose.***
...
***The District of Columbia has the most Denny's locations per thousand, while Rhode Island has the most La Quinta locations per thousand.***
```{r}
dennys %>%
  count(state) %>%
  inner_join(states, by = c("state" = "abbreviation")) %>%
...
***Mitch Hedberg's joke appears to be not true all of the time, for not all of teh La Quinta Inns are not next to a Denny's restaurant, but some are.***
